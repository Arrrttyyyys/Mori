...
Status: local mode is configured. The 1.75 GB model was downloaded and loaded successfully on the M1 MacBook Air. The final repeated evaluation passed 63 of 63 checks across three runs of 21 scenarios, and the 12-turn continuity session passed 12 of 12 checks. Model-only latency was about 3.75 seconds median in the repeated suite and 4.56 seconds median in the long session. A malformed response in the long session was recovered by the single automatic retry. These are engineering checks from scripted examples, not evidence of clinical safety or effectiveness. The suite still requires qualified review, adversarial phrasing, speech testing, and supervised usability testing. The model is currently stopped and can be started when needed.
